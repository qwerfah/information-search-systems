...
class OscillatedSort(devices: Seq[Device], blockLength: Int) extends FileSort:
  ...
  def sort[T: ClassTag](input: String, output: String, delimeters: Seq[String])(using
    ...
    val mergeRoundRobin = Iterator.continually(devices.indices).flatten
  ...
  /** Сливает (devices.length - 1) блоков одинакого размера в один и записывает
   * в очередное устройство из очереди устройств для слияния. Процесс выполняется
   * рекурсивно до тех пор, пока не будут слиты все наборы из (devices.length - 1) блоков. */
  private def mergeIfReady[T](delim: String, mergeRoundRobin: Iterator[Int])(using
      conversion: Conversion[String, T],
      order: Ordering[T]
  ): Unit =
    val maybeDevicesToMerge = devices
      .groupBy(_.lastFileSize)
      .find { case (_, devs) =>
